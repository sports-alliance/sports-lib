...
var Activity = /** @class */ (function (_super) {
    ...
    Activity.prototype.getDataLength = function () {
        return Math.ceil((+this.endDate - +this.startDate) / 1000);
    };
    Activity.prototype.createStream = function (type) {
        return new stream_1.Stream(type, Array(this.getDataLength()).fill(null));
    };
    ...
    Activity.prototype.getStreamDataBasedOnDataType = function (streamTypeToBaseOn, streamTypes) {
        var _this = this;
        return this.getStreamData(streamTypeToBaseOn).reduce(function (accu, streamDataItem, index) {
            if (!helpers_1.isNumber(streamDataItem)) {
                return accu;
            }
            _this.getAllStreams()
                ...
                if (_this.getStreamData(stream.type)[index]) {
                    accu[streamDataItem] = accu[streamDataItem] || {};
                    accu[streamDataItem][stream.type] = _this.getStreamData(stream.type)[index];
                }
            });
            return accu;
        }, {});
    };
    Activity.prototype.getStreamDataBasedOnTime = function (streamTypes) {
        var _this = this;
        var streamDataBasedOnTime = {};
        var _loop_1 = function (i) {
            streamTypes.forEach(function (streamType) {
                if (helpers_1.isNumber(_this.getStreamData(streamType)[i])) {
                    streamDataBasedOnTime[_this.startDate.getTime() + (i * 1000)] = streamDataBasedOnTime[_this.startDate.getTime() + (i * 1000)] || {};
                    streamDataBasedOnTime[_this.startDate.getTime() + (i * 1000)][streamType] = _this.getStreamData(streamType)[i];
                }
            });
        };
