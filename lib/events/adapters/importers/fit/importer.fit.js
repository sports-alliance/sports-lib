...
var easy_fit_1 = __importDefault(require("easy-fit"));
...
var EventImporterFIT = /** @class */ (function () {
    ...
    EventImporterFIT.getFromArrayBuffer = function (arrayBuffer) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var easyFitParser = new easy_fit_1.default({
                force: false,
                speedUnit: 'm/s',
                lengthUnit: 'm',
            ...
            easyFitParser.parse(arrayBuffer, function (error, fitDataObject) {
                ...
                fitDataObject.activity.sessions.forEach(function (sessionObject) {
                    ...
                    var activity = _this.getActivityFromSessionObject(sessionObject);
                    // Set the creator to the activity
                    activity.creator = _this.getCreatorFromFitDataObject(fitDataObject);
                    // Go over the laps
    ...
    EventImporterFIT.getLapFromSessionLapObject = function (sessionLapObject) {
        ...
        // Set the type
        lap.type = lap_types_1.LapTypes[sessionLapObject.lap_trigger];
        // Add stats to the lap
        this.getStatsFromObject(sessionLapObject).forEach(function (stat) { return lap.addStat(stat); });
        return lap;
    };
    EventImporterFIT.getActivityFromSessionObject = function (sessionObject) {
        // Create an activity
        var activity = new activity_1.Activity(sessionObject.start_time, sessionObject.timestamp || new Date(sessionObject.start_time.getTime() + sessionObject.total_elapsed_time * 1000));
        // Set the type
    ...
    EventImporterFIT.getCreatorFromFitDataObject = function (fitDataObject) {
        var creator = new creator_1.Creator();
        ...
        switch (fitDataObject.file_id.manufacturer) {
            case 'suunto': {
                creator.name = importer_fit_suunto_device_names_1.ImporterFitSuuntoDeviceNames[fitDataObject.file_id.product];
                break;
            }
            case 'garmin': {
                creator.name = importer_fit_garmin_device_names_1.ImporterFitGarminDeviceNames[fitDataObject.file_id.product];
                break;
            }
            case 'zwift': {
                creator.name = importer_fit_swift_device_names_1.ImporterZwiftDeviceNames[fitDataObject.file_id.product];
                break;
            }
        }
        if (!creator.name) {
