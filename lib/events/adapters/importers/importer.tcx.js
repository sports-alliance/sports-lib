...
var EventImporterTCX = /** @class */ (function () {
    ...
    EventImporterTCX.getFromXML = function (xml) {
        var _this = this;
        // Init the event
        ...
        var _loop_1 = function (activityElement) {
            ...
            var activity = new activity_1.Activity(new Date(activityElement.getElementsByTagName('Lap')[0].getAttribute('StartTime')), laps[laps.length - 1].endDate);
            event.addActivity(activity);
            // Set the type
            activity.type = activity_types_1.ActivityTypes[activityElement.getAttribute('Sport')] || activity_types_1.ActivityTypes['unknown'];
            // Setup the creator
            activity.creator = this_1.getCreator(activityElement.getElementsByTagName('Creator')[0]);
            ...
            laps.forEach(function (lap) {
                ...
                activity.getStat(data_energy_1.DataEnergy.className).setValue(activity.getStat(data_energy_1.DataEnergy.className).getValue() + lap.getStat(data_energy_1.DataEnergy.className).getValue());
