...
var EventUtilities = /** @class */ (function () {
    ...
    EventUtilities.getDataTypeAverage = function (event, dataType, startDate, endDate, activities) {
        ...
        var averageForDataType = event.getPoints(startDate, endDate, activities).reduce(function (average, point) {
            ...
            var value = Number(point.getDataByType(dataType).getValue());
    ...
    EventUtilities.mergeEvents = function (events) {
        return new Promise(function (resolve, reject) {
            ...
            var mergeEvent = new event_1.Event();
    ...
    EventUtilities.getEventDataTypeGainOrLoss = function (gain, event, dataType, starDate, endDate, activities, minDiff) {
        ...
        points.reduce(function (previous, next) {
            ...
            if (gain) {
                ...
                if ((previous.getDataByType(dataType).getValue() + minDiff) <= next.getDataByType(dataType).getValue()) {
                    gainOrLoss += next.getDataByType(dataType).getValue() - previous.getDataByType(dataType).getValue();
                    return next;
                }
                ...
                if (previous.getDataByType(dataType).getValue() <= next.getDataByType(dataType).getValue()) {
                    return previous;
                }
                return next;
            }
            ...
            if ((previous.getDataByType(dataType).getValue() - minDiff) >= next.getDataByType(dataType).getValue()) {
                gainOrLoss += previous.getDataByType(dataType).getValue() - next.getDataByType(dataType).getValue();
                return next;
            }
            // if not eligible check if smaller without the diff and if yes do not register it and send it back as the last to check against
            if (previous.getDataByType(dataType).getValue() >= next.getDataByType(dataType).getValue()) {
    ...
    EventUtilities.getDataTypeMinOrMax = function (max, event, dataType, startDate, endDate, activities) {
        ...
        if (max) {
            return dataValuesArray.length ? Math.max.apply(Math, dataValuesArray) : null;
        }
        return dataValuesArray.length ? Math.min.apply(Math, dataValuesArray) : null;
    };
