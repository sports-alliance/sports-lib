...
var EventUtilities = /** @class */ (function () {
    ...
    EventUtilities.reGenerateStatsForEvent = function (event) {
        ...
        event.addStat(new data_energy_1.DataEnergy(0));
        event.getActivities().forEach(function (activity) {
        ...
        event.getActivities().forEach(function (activity) {
            ...
            if (activityEnergy) {
                var energy = event.getStat(data_descent_1.DataDescent.type);
                if (!energy) {
                    event.addStat(new data_energy_1.DataEnergy(activityEnergy.getValue()));
                }
                else {
                    event.addStat(new data_energy_1.DataEnergy(energy.getValue() + activityEnergy.getValue()));
                }
            }
            var activityAVGHeartRate = activity.getStat(data_heart_rate_avg_1.DataHeartRateAvg.type);
            if (activityAVGHeartRate) {
                var avgHR = event.getStat(data_heart_rate_avg_1.DataHeartRateAvg.type);
                if (!avgHR) {
                    event.addStat(new data_heart_rate_avg_1.DataHeartRateAvg(activityAVGHeartRate.getValue()));
                }
                else {
                    event.addStat(new data_heart_rate_avg_1.DataHeartRateAvg((avgHR.getValue() + activityAVGHeartRate.getValue()) / 2));
                }
            }
            var activityAVGPower = activity.getStat(data_power_avg_1.DataPowerAvg.type);
            if (activityAVGPower) {
                var avgPower = event.getStat(data_power_avg_1.DataPowerAvg.type);
                if (!avgPower) {
                    event.addStat(new data_power_avg_1.DataPowerAvg(activityAVGPower.getValue()));
                }
                else {
                    event.addStat(new data_power_avg_1.DataPowerAvg((avgPower.getValue() + activityAVGPower.getValue()) / 2));
                }
            }
